...
| [`odradek-sasl`](crates/odradek-sasl) | Sans-I/O SASL for these crates: SCRAM-SHA-256/512 and PLAIN, both roles, checked against the RFC 7677 vectors. |
...
- `odradek-client`: framed connection with correlation-id pipelining and
  ...
  and — since the examples stopped being something a person had to
  remember to run — against every subject of the broker matrix in CI:
  `group_consume` and
...
- `odradek-acceptance`: conformance checks for **both roles** over raw
  connections, independent of the client crate. 62 server checks across
