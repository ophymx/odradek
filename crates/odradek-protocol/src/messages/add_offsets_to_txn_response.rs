//! `AddOffsetsToTxnResponse` — generated from `schemas/AddOffsetsToTxnResponse.json`.
//!
...
/// `AddOffsetsToTxnResponse`, versions 0-4.
...
impl AddOffsetsToTxnResponse {
    ...
    /// Encode this `AddOffsetsToTxnResponse` at `version`.
    ...
    /// Decode one `AddOffsetsToTxnResponse`, bounding allocation by the default
    ...
    /// Decode one `AddOffsetsToTxnResponse` under `limits`.
    ...
    /// Decode one `AddOffsetsToTxnResponse` against an existing `budget`.
...
impl crate::Message for AddOffsetsToTxnResponse {
    ...
    fn encode(&self, buf: &mut impl BufMut, version: i16) -> Result<(), EncodeError> {
        AddOffsetsToTxnResponse::encode(self, buf, version)
    }
    fn decode(buf: &mut impl Buf, version: i16) -> Result<Self, DecodeError> {
        AddOffsetsToTxnResponse::decode(buf, version)
    }
    fn decode_with_limits(
        ...
        AddOffsetsToTxnResponse::decode_with_limits(buf, version, limits)
    }
